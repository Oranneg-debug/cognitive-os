...
**Status**: ✅ Approved - Technical Council Meeting  
...
## ✅ Approved Architecture (Post-Council Revision)

### Final Decision: Option A (Sync Script) – Enhanced for Windows
- **Primary Workspace**: `E:\Antigravity\cognitive-os\dev` (local dev)
- **Vault Integration**: `.obsidian/plugins/dev/` (read-only sync target)
- **Key Features**:
  - Uses `robocopy` (Windows-native) for reliable, incremental file synchronization
  - Planning docs (`proposals/`, `decisions/`) are version-controlled but excluded from build artifacts
  - Git ignores generated files in the dev folder to prevent drift

### Sync Script (`cognitive-os/sync-to-vault.bat`)
```batch
@echo off
...
- Vault dev folder is git-tracked but .gitignore'd for portability
- Build artifacts must be cleaned post-sync
...
- [x] Approved by Technical Council (2026-05-18)
...
| Date | Decision | Notes |
|------|----------|-------|
| 2026-05-18 | Under Review | Initial proposal |
| 2026-05-18 | **Approved** | Technical Council Meeting - Revised architecture with robocopy sync |

---

*Proposal stored at: cognitive-os/dev/proposals/2026-05-18_OBSIDIAN_DEV_FLOW.md*