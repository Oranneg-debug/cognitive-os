...
def test_main_apply_creates_sqlite_and_returns_zero(
    ...
    Note we pass --decisions-dir so the migration report lands under
    tmp_path; otherwise a real test run would pollute the production
    ``dev/decisions/`` folder. This is the same isolation bug that
    leaked two reports during the first B1 commit's test run.
    """
    db = tmp_path / "kanban.sqlite"
    ...
    assert rc == 0
    assert db.exists()
    # Migration report landed in tmp_path, not in real dev/decisions/
    reports = list(decisions.glob("_kanban_migration_*.md"))
